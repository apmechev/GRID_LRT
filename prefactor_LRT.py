...
def setup_dirs():
	...
	for dir in ['Tokens','Staging']:
			...
			cfgfile.write("LOFARDIR = "+d_vars["sw_dir"]+"/"+d_vars["sw_ver"]+'\n')
...
def check_state_and_stage():
	if "Initial" in d_vars["parsetfile"]:
		print "Nothing to stage for Initial_Subtract steps"
		return
...
#	replace_in_file("prefactor-refactor.sh","SW_DIR=/cvmfs/softdrive.nl/wjvriend/lofar_stack","SW_DIR="+d_vars["sw_dir"]) These are now done in the createtoken.py
#	replace_in_file("prefactor-refactor.sh","2.16",d_vars["sw_ver"])
#	replace_in_file("pipeline.cfg","/cvmfs/softdrive.nl/wjvriend/lofar_stack/2.16",d_vars["sw_dir"]+"/"+d_vars["sw_ver"])
	print "MODIFYING prefactor-refactor.sh!"
	shutil.copy("../../../"+d_vars["parsetfile"],"prefactor/"+d_vars["parsetfile"])
